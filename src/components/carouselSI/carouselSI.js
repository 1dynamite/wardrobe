...
const bgTexts = ['Lorem ipsum dolor sit 1', 'Lorem ipsum dolor sit 2', 'Lorem ipsum dolor sit 3'];
...
const CarouselSI = (props) => {  
  ...
  const [imgContainerStyles1, setImgContainerStyles1] = useState(imgContainerStylesDefault1);
  const [imgContainerStyles2, setImgContainerStyles2] = useState(imgContainerStylesDefault2);
  ...
  useEffect(() => {
    ...
    myTimeout = setTimeout(timeoutFunc, 3000);
  ...
  useEffect(() => {
    ...
    const newImgCOpacity1 = imgCOpacity1 === opacityUpperBound ? '0' : opacityUpperBound;
  ...
  const switchImgTag = () => {
    ...
    const next = bgImages.length !== index + 1 ? bgImages[index+1] : bgImages[0];
    ...
    const nextText = bgTexts.length !== index + 1 ? bgTexts[index+1] : bgTexts[0];

    if(imgCOpacity1 === '0')
      setImgText1(nextText);
    else
      setImgText2(nextText);

  };


  useEffect(() => {
    setImgContainerStyles1(css(
      {
